...
interface CommandPaletteProps {
  ...
  onOpenGeminiCLI: () => void;
}

interface Command {
  id: string;
  type: 'navigation' | 'project' | 'action' | 'ai';
  title: string;
  category: string;
  action: () => void;
  keywords?: string;
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ user, onClose, setActiveView, onOpenGeminiCLI }) => {
    ...
    const allCommands = useMemo<Command[]>(() => {
        ...
        const aiCommands: Command[] = [
            { 
                ...
                action: () => onOpenGeminiCLI(), 
                keywords: 'gemini ai assistant cli terminal command', 
                icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 9l3 3-3 3m5 0h3" /></svg>
            }
        ];
        ...
        return [...navCommands, ...aiCommands, ...projectCommands];
    }, [projects, setActiveView, onOpenGeminiCLI]);
